...
def pre(userInput):
    ...
    for row in range(len(x['criteria'])):
        ...
        try:            
            ...
            #innerFrame.config(bg="black")
            printToUser("Testing", end = '')
...
#   on "Enter" key, get user input    
def onclick(event):
    msg = entry.get()
    messageScreen.config(state=NORMAL)
    messageScreen.insert(END, msg + "\n")
    messageScreen.config(state=DISABLED)    
    chatuserLoop()
...
def printToUser(chatbotOutput):
    messageScreen.config(state=NORMAL)
    messageScreen.insert(END, chatbotOutput + "\n")
    messageScreen.config(state=DISABLED)
...
if __name__ == "__main__":
    ...
    root = tk.Tk()
    root.title("Notts-Cafeteria Chatbot")
    root.geometry("600x400+1250+450")
    root.resizable(False, False)
    root.wm_attributes("-topmost", 1)

    #Create padding
    innerFrame = tk.Frame(root, borderwidth=25, bg="#ffd1dc")
    innerFrame.pack(fill="both", expand=True)

    #Create screen for messages
    messageScreen = Text(innerFrame, height=5, width=52, bg="white")
    messageScreen.place(relheight=0.745, relwidth=1)
    messageScreen.config(cursor="arrow", state=DISABLED)

    #Create scrollbar 
    #scrollBar = Scrollbar(messageScreen)
    #scrollBar.place(relheight=1, relx=0.97)
    #scrollBar.config(cursor="hand2", command=messageScreen.yview)

    #Create input box
    entry = tk.Entry(innerFrame, font=('helvetica',10,'normal'), bd=5, relief=GROOVE)
